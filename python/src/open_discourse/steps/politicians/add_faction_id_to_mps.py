...
def load_required_data(
    ...
    logger.info(f"Loaded {len(politicians_df)} politician records and {len(factions_df)} faction records")
    return politicians_df, factions_df

def add_faction_ids(politicians_df: pd.DataFrame, factions_df: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    for faction_name, faction_id in zip(factions_df["faction_name"], factions_df["id"]):
        ...
        if match_count_this_faction > 0:
            ...
            logger.debug(f"Matched {match_count_this_faction} politicians to faction '{faction_name}' (ID: {faction_id})")
    ...
    if unmatched_count > 0:
        logger.warning(f"{unmatched_count} politicians could not be matched to any faction")
