...
# Constants
WIKIPEDIA_URL = "https://de.wikipedia.org/wiki/Liste_der_deutschen_Regierungsmitglieder_seit_1949"
...
def parse_government_position(position_text: str) -> Tuple[Optional[str], int, int]:
    ...
    if len(match_years) == 2:
        ...
        if len(title_part) > 1:
            ...
            return position, position_from1, position_until1, position_from2, position_until2
        else:
            raise ValueError(f"Cannot extract position title from multiple terms: {position_text}")
    else:
...
def extract_politician_data(content_div: BeautifulSoup) -> pd.DataFrame:
            ...
            for ul in div.find_all("ul", recursive=False):
                for li in ul.find_all("li", recursive=False):
                    try:
                        ...
                        if "CDU" in name:
                            logger.debug(f"Skipping problematic entry with 'CDU' in name: {name}")
                            continue
                        ...
                        birth_date, death_date = extract_birth_death_years(birth_death_text)
                        
                        # Process each government position for this politician
                        positions_list = li.find_all("li")
                        if not positions_list:
                            logger.warning(f"No positions found for {first_name} {last_name}, skipping")
                            continue
                        ...
                        for pos_item in positions_list:
                            ...
                            try:
                                ...
                                if len(result) == 5:
                                    position, pos_from1, pos_until1, pos_from2, pos_until2 = result
                                    ...
                                    politicians_data["position_until"].append(pos_until1)
                                    ...
                                    politicians_data["additional_faction"].append(additional_faction)
                                ...
                                politicians_data["position_until"].append(position_until)
                                politicians_data["birth_date"].append(birth_date)
                                politicians_data["death_date"].append(death_date)
                                politicians_data["faction"].append(faction)
                                politicians_data["additional_faction"].append(additional_faction)
                                ...
                                logger.error(f"Error parsing position for {first_name} {last_name}: {e}")
                                continue
    ...
    logger.info(f"Extracted data for {unique_politicians} politicians with {total_positions} government positions")
...
def main(task) -> bool:
    ...
    try:
        ...
        if politicians_df.empty:
            logger.error("Failed to extract politicians data, resulting DataFrame is empty")
            return False
    except Exception as e:
