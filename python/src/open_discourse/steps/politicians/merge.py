...
def load_politicians_data(
    ...
    logger.info(
        f"Loaded {len(mps_df)} MPs, {len(mgs_df)} government members, and {len(factions_df)} factions"
    )
...
def _fix_special_case_names(
    last_name: str, first_names: List[str]
...
def create_government_position_entry(
    politician_data: Dict[str, Any], electoral_term: int, faction_id: int, position: str
) -> Dict[str, Any]:
    """
...
def merge_government_members(
    politicians_df: pd.DataFrame, mgs_df: pd.DataFrame, factions_df: pd.DataFrame
) -> pd.DataFrame:
    """
    ...
    for (
        ...
        try:
            ...
            first_names, faction_override = _fix_special_case_names(
            ...
            if faction_abbrev:
                try:
                    faction_match = int(
                        factions_df.loc[
                            factions_df["abbreviation"] == faction_abbrev, "id"
                        ].iloc[0]
                    )
                except (IndexError, KeyError):
                    logger.warning(
                        f"Could not find faction ID for abbreviation: {faction_abbrev}"
                    )
                    faction_match = -1
            else:
            ...
            try:
                ...
                logger.error(
                    f"Error determining electoral terms for position {position}: {e}"
                )
                continue
            ...
            # Case 2: Multiple matches (shouldn't happen with current logic)
                ...
                logger.warning(
                    f"Multiple matches found for {' '.join(first_names)} {last_name}"
                )
                # Use the first match
                politician_data = possible_matches.iloc[0]
            ...
            # Case 3: No match found - create new politician entry
                ...
                max_ui = max(
                    [
                        int(ui) if str(ui).isdigit() else 0
                        for ui in result_df["ui"].tolist()
                    ]
                )
            ...
            logger.error(
                f"Error processing government member {first_name} {last_name}: {e}"
            )
            continue
...
def main(task) -> bool:
    ...
    mps_df, mgs_df, factions_df = load_politicians_data(
        MPS_PATH, MGS_PATH, FACTIONS_PATH
    )
    ...
    try:
        ...
        logger.info(
            f"Successfully merged politicians data, final size: {len(merged_politicians)} records"
        )
    except Exception as e:
