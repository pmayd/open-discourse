...
class TestAddFactionIdToMPs:
    ...
    def test_load_required_data_success(
        self, mock_load_pickle, mock_politicians_df, mock_factions_df
    ):
        ...
        # Call the function
        politicians_df, factions_df = load_required_data(
            Path("/test/politicians"), Path("/test/factions")
        )
    ...
    @patch("open_discourse.steps.politicians.add_faction_id_to_mps.load_pickle")
    def test_load_required_data_factions_missing_columns(
        self, mock_load_pickle, mock_politicians_df
    ):
    ...
    @patch("open_discourse.steps.politicians.add_faction_id_to_mps.load_pickle")
    def test_load_required_data_politicians_missing_columns(
        self, mock_load_pickle, mock_factions_df
    ):
    ...
    def test_add_faction_ids(self, mock_politicians_df, mock_factions_df):
        ...
        assert (
            result_df.loc[result_df["institution_name"] == "SPD", "faction_id"].iloc[0]
            == 1
        )
        assert (
            result_df.loc[
                result_df["institution_name"] == "CDU/CSU", "faction_id"
            ].iloc[0]
            == 2
        )
        assert (
            result_df.loc[result_df["institution_name"] == "FDP", "faction_id"].iloc[0]
            == 3
        )
        assert (
            result_df.loc[
                result_df["institution_name"] == "Unknown", "faction_id"
            ].iloc[0]
            == -1
        )

    def test_add_faction_ids_with_existing_column(
        self, mock_politicians_df, mock_factions_df
    ):
    ...
    def test_main_success(
        self,
        mock_save_pickle,
        mock_add_faction_ids,
        mock_load_required_data,
        mock_politicians_df,
        mock_factions_df,
    ):
        ...
        mock_add_faction_ids.assert_called_once_with(
    ...
    @patch("open_discourse.steps.politicians.add_faction_id_to_mps.load_required_data")
    @patch("open_discourse.steps.politicians.add_faction_id_to_mps.add_faction_ids")
    def test_main_add_faction_ids_failure(
        self,
        mock_add_faction_ids,
        mock_load_required_data,
        mock_politicians_df,
