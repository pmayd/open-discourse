...
class TestMergePoliticians:
    ...
    def test_load_politicians_data_success(
        self, mock_load_pickle, mock_mps_df, mock_mgs_df, mock_factions_df
    ):
    ...
    @patch("open_discourse.steps.politicians.merge_politicians.load_pickle")
    def test_load_politicians_data_missing_columns(
        self, mock_load_pickle, mock_mps_df, mock_mgs_df, mock_factions_df
    ):
    ...
    def test_create_government_position_entry_from_series(self, mock_mps_df):
        ...
        entry = create_government_position_entry(
            politician_data, electoral_term, faction_id, position
        )
    ...
    def test_merge_government_members(self, mock_mps_df, mock_mgs_df, mock_factions_df):
        ...
        with patch("open_discourse.steps.politicians.merge_politicians.tqdm"):
            ...
            result_df = merge_government_members(
    ...
    @patch(
        "open_discourse.steps.politicians.merge_politicians.merge_government_members"
    )
    def test_main_success(
        self,
        mock_merge_govt,
        mock_load_politicians,
        mock_mps_df,
        mock_mgs_df,
        mock_factions_df,
    ):
        ...
        # Setup mocks
        mock_load_politicians.return_value = (
            mock_mps_df,
            mock_mgs_df,
            mock_factions_df,
        )
        ...
        with patch.object(pd.DataFrame, "to_csv") as mock_to_csv:
            ...
            mock_merge_govt.assert_called_once_with(
                mock_mps_df, mock_mgs_df, mock_factions_df
            )
